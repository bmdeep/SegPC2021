...
def unet(pretrained_weights = None,input_size = (256,256,1), LR = 1e-4):
    ...
    conv11 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(merge9)
...
def unet_dns(input_shape, pretrained_weights=None, LR=1e-4, out_ch=1):
...
def joint_net(input_size = (256, 256, 1), LR = 1e-4, OUt_ch = 1):
    inputs = Input(input_size)
    conv1 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(inputs)
    conv1 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
    conv2 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(pool1)
    conv2 = Conv2D(128, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv2)
    pool2 = UpSampling2D(size = (2,2))(conv2)
...
if __name__ == "__main__":
    ...
    model.fit(x,y,
              ...
              shuffle=True,
               verbose=1,)
    print('fine') 
    model_j = joint_net(input_size = (patch_h, patch_w, 5), LR = 1e-4)
    model_j.summary()
    



def unet_dnspro(input_shape, pretrained_weights=None, LR = 1e-4, out_ch = 1):
    inputs = Input(input_shape)
    conv1 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(inputs)
    conv1 = Conv2D(64, 3, activation = 'relu', padding = 'same', kernel_initializer = 'he_normal')(conv1)
    pool1 = MaxPooling2D(pool_size=(2, 2))(conv1)
